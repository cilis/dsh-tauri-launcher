...
`
    ...
    // 消息统一信封 { v, type, payload }。为兼容旧版对端（旧 exe 配新插件，
    // 或反之），发送时同时保留旧字段、解析时信封优先并回退旧字段；下个大
    // 版本再移除旧字段。字段与超时清单见项目 docs/architecture.md。
    const MSG_VERSION = 1
    const MSG = {
      /** 外壳 → 插件：导航命令（dir: back | forward | ping）。 */
      NAV_COMMAND: 'navCommand',
      /** 外壳 → 插件：Windows 系统主题（scheme）。 */
      SYSTEM_THEME: 'systemTheme',
      /** 插件 → 外壳：前进/后退可用性（back / forward）。 */
      NAV_STATUS: 'navStatus',
      /** 插件 → 外壳：DSH 主题与真实 token（scheme + 六色）。 */
     ...
     * ⚠️ 所有读取都在当前任务之后（见下方 schedulePost 的时序说明）——
     * 同步读会拿到上一个主题的 DOM 状态，标题栏会滞后一个主题。
     */
    function installThemeRelay(ctx) {
      ...
      const postTheme = () => {
        ...
        // 同一状态只投递一次：事件、DOM 观察、重试梯子三路触发时去重
        if (signature === lastSignature) return true
        lastSignature = signature
        // 双发：信封（新）+ 旧字段（兼容旧版外壳）；下个大版本移除旧字段。
        window.parent.postMessage(
          { ...theme, __dshLauncherTheme: 1, v: MSG_VERSION, type: MSG.THEME_SYNC, payload: theme },
          '*',
        )
        // 返回 true 供重试梯子判定“已送达”，停止后续重试
        return true
      }

      /**
       * 读取必须延后到当前任务之后（DSH 0.1.2-rc.1 实测的时序坑）：
       * ui-layout 的 ThemePresenter 与我们的 theme/change 监听器同处一次
       * **同步**派发，谁先注册不保证；同步读取会拿到上一个主题的 body 属性
       * 与 --dsw-* 计算值——表现为「DSH 界面已切浅色，标题栏仍是深色」
       * （标题栏滞后一个主题）。setTimeout(0) 保证 presenter 的 DOM 写入生效。
       */
      const schedulePost = () => {
      ...
      try {
        ctx.on('theme/change', schedulePost)
      } catch { /* 事件不可用时依赖下面的重试梯子 */ }

      // 兜底：直接观察 body 的主题属性，事件时序变化时也能补发（同状态去重）
      if (typeof MutationObserver === 'function') {
        const observer = new MutationObserver(() => schedulePost())
        observer.observe(document.body, { attributes: true, attributeFilter: ['data-ds-dark-theme'] })
        ctx.effect(() => () => observer.disconnect(), 'dsh-tauri-launcher: theme relay observer')
      }

      // 主题服务可能在本插件之后才就绪，且 preference 等于默认值时不会发
      // theme/change：用重试梯子保证初始状态一定送达（成功或 6 秒后停止）。
      const disposeTimer = retryLadder(schedulePost, {})
    ...
    function installSystemThemeFollow(ctx, parentOrigin) {
      ...
      const onSystemThemeMessage = (event) => {
        ...
        if (!isShellMessage(event, parentOrigin)) return
    ...
    function installSessionNav(ctx, parentOrigin) {
      ...
      const initNav = () => {
        ...
        const sendStatus = () => {
          ...
          const status = { back: nav.status().back, forward: nav.status().forward }
          try {
            // 双发：信封（新）+ 旧字段（兼容旧版外壳）。用 '*' 投递：WebView2 下
            // 对 http://tauri.localhost 虚拟主机的精确 targetOrigin 匹配有丢弃
            // 嫌疑；外壳侧会校验 event.origin。
            window.parent.postMessage(
              { __tbNavStatus: status, v: MSG_VERSION, type: MSG.NAV_STATUS, payload: status },
              '*',
            )
          } catch { /* 忽略 */ }
        }
     ...
     * 推导。**注意 referrer 不可靠**：DSH 0.1.5 + WebView2 实测，iframe 经
     * token 303 握手后 `document.referrer` 为空，推导结果退化为兜底值，
     * 与外壳实际 origin 不符——因此校验一律走 [`isShellMessage`] 的白名单。
     */
    function deriveParentOrigin() {
      try {
        if (document.referrer) return new URL(document.referrer).origin
      } catch { /* 兜底见 isShellMessage 的白名单 */ }
      return 'http://tauri.localhost'
    }

    /**
     * 外壳页（桌面端主窗口）的已知 origin：本地外壳服务（`shell_server.rs`，
     * 127.0.0.1:3081）与历史 tauri 资产协议虚拟主机。
     */
    const SHELL_ORIGINS = ['http://127.0.0.1:3081', 'http://tauri.localhost']

    /**
     * 判断消息是否来自桌面端外壳。
     * `event.source === window.parent` 是最强的身份校验（只有真正的父 frame
     * 能匹配）；origin 再按「referrer 推导值或已知外壳白名单」收口——只认
     * 推导值会在 referrer 缺失时把整条通道（导航命令、系统主题）全部丢弃
     * （2026-09-12 探针实证：34 次 ping 与全部 nav/theme 消息均因 origin
     * 不匹配被丢弃，症状是 ◀/▶ 无响应、Windows 主题不跟随）。
     */
    function isShellMessage(event, parentOrigin) {
      if (event.source !== window.parent) return false
      return event.origin === parentOrigin || SHELL_ORIGINS.includes(event.origin)
    }

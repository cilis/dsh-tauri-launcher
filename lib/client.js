...
`
    ...
    exports.apply = function (ctx) {
      ...
      // ---------- 标题栏主题中继 ----------
      // 桌面端把 DSH 嵌在 iframe 里；这里读取官方主题服务的解析快照（active.colorScheme
      // 与侧栏底色 token --dsw-specific-sidebar-fill），postMessage 发给启动器外壳，
      // 让标题栏跟随 DSH 浅/深主题。仅当本页面被嵌入（iframe）时才有意义，
        ...
        try {
          ...
          if (theme && typeof theme.getTheme === 'function') {
            ...
            if (scheme === 'light' || scheme === 'dark') {
              const tokens = (snap.active && snap.active.tokens) || {}
              const bg = tokens['--dsw-specific-sidebar-fill']
              payload = {

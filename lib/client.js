...
`
    ...
    exports.apply = function (ctx) {
      ...
      // 栈条目随行记录 address，子会话经 openSubagent 跳转。仅在 iframe 内嵌时启用。
      if (window.parent !== window) {
        ...
        if (store && typeof store.subscribe === 'function') {
          ...
          const sendStatus = () => {
            ...
            try {
              window.parent.postMessage(
                { __tbNavStatus: { back: backStack.length > 0, forward: forwardStack.length > 0 } },
                parentOrigin,
              )
            } catch { /* 忽略 */ }
          }
          ...
          const onListChange = () => {
            ...
            // 用户级切换：上一个会话压 backStack，清空 forwardStack

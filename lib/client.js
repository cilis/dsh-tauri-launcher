...
`
    ...
    function installSystemThemeFollow(ctx, parentOrigin) {
      ...
      const onSystemThemeMessage = (event) => {
        ...
        const payload = readEnvelope(data, MSG.SYSTEM_THEME)
        const scheme = payload ? payload.scheme : data.__tbSystemTheme
    ...
    function installSessionNav(ctx, parentOrigin) {
      ...
      const initNav = () => {
        ...
        const onNavMessage = (event) => {
          ...
          if (event.source !== window.parent) return
          if (event.origin !== parentOrigin) return
          const payload = readEnvelope(data, MSG.NAV_COMMAND)
          const dir = payload ? payload.dir : data.__tbNav
          if (dir === 'back') goBack()

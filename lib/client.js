...
`
    ...
    exports.apply = function (ctx) {
      ...
      // 这里维护"最近访问会话"栈：非本插件发起的 current 变化 → 上一个会话压
      // backStack、清空 forwardStack；◀ 弹 backStack 跳转（当前压 forwardStack），
      // ▶ 反向。跳转前设 pendingJump 锁，防止自身触发的变化被重复压栈，超时兜底解锁。
      // sessions 服务存在就绪时序：一次性 ctx.get 可能拿到 undefined（静默失效），
      // 故用重试梯子初始化；全链路带运行时诊断（POST /nav-diag 写宿主日志）。
      if (window.parent !== window) {
        let navDispose = null
        let navReady = false

        const navDiag = (stage, detail) => {
          try {
            void fetch(API_BASE + '/nav-diag', {
              method: 'POST',
              headers: { ...CSRF_HEADERS, 'content-type': 'application/json' },
        ...
        // 父窗口 origin：跨源读不到 window.parent.location，用 document.referrer 推导
        let parentOrigin = 'http://tauri.localhost'
        try {
          if (document.referrer) parentOrigin = new URL(document.referrer).origin
        } catch { /* 兜底 http://tauri.localhost */ }
        navDiag('nav-init', 'parent=' + (window.parent !== window) + ' referrer=' + document.referrer + ' parentOrigin=' + parentOrigin)

        const initNav = () => {
          const sessions = ctx.get('sessions')
          const store = sessions && sessions.list
          if (!store || typeof store.subscribe !== 'function') {
            navDiag('nav-init-missing', 'sessions=' + !!sessions + ' list=' + !!store)
            return false
          }
          ...
          const sendStatus = () => {
            if (window.parent === window) return
            const payload = { __tbNavStatus: { back: backStack.length > 0, forward: forwardStack.length > 0 } }
            try {
              window.parent.postMessage(payload, parentOrigin)
            } catch { /* 忽略 */ }
            navDiag('nav-status', 'back=' + payload.__tbNavStatus.back + ' forward=' + payload.__tbNavStatus.forward + ' targetOrigin=' + parentOrigin)
          }
          ...
          const jumpTo = (entry) => {
            ...
            jumpTimer = setTimeout(() => {
              ...
              navDiag('nav-jump-timeout', 'fallback snapshot id=' + snap.id)
              sendStatus()
            }, 1500)
            try {
              if (entry.address) sessions.openSubagent(entry.address)
              else sessions.open(entry.id)
              navDiag('nav-jump', 'via=' + (entry.address ? 'openSubagent' : 'open') + ' id=' + entry.id + ' address=' + JSON.stringify(entry.address))
            } catch (error) {
              pendingJump = null
              navDiag('nav-jump-error', String((error && error.message) || error))
            }
          }

          const goBack = () => {
            navDiag('nav-go-back', 'backStack=' + backStack.length + ' forwardStack=' + forwardStack.length + ' current=' + currentId)
          ...
          const goForward = () => {
            navDiag('nav-go-forward', 'backStack=' + backStack.length + ' forwardStack=' + forwardStack.length + ' current=' + currentId)
          ...
          const onListChange = () => {
            const snap = snapCurrent()
            navDiag('nav-list-change', 'id=' + snap.id + ' address=' + JSON.stringify(snap.address) + ' currentId=' + currentId + ' pending=' + (pendingJump ? pendingJump.id : 'null'))
            if (pendingJump) {
          ...
          const onNavMessage = (event) => {
            ...
            navDiag('nav-message', 'dir=' + dir + ' origin=' + event.origin + ' sourceIsParent=' + (event.source === window.parent) + ' originOk=' + (event.origin === parentOrigin))
          ...
          const onKeydown = (event) => {
            ...
            if (event.key === 'ArrowLeft') {
              ...
              navDiag('nav-key', 'Alt+ArrowLeft')
              goBack()
            } else if (event.key === 'ArrowRight') {
              event.preventDefault()
              navDiag('nav-key', 'Alt+ArrowRight')
              goForward()
            }
          }
          ...
          const onMouseup = (event) => {
            if (event.button === 3) {
              event.preventDefault()
              navDiag('nav-mouse', 'button3 back')
              goBack()
            } else if (event.button === 4) {
              event.preventDefault()
              navDiag('nav-mouse', 'button4 forward')
              goForward()
            }
          }
          ...
          navDiag('nav-ready', 'subscribe=' + typeof unsubscribe + ' init id=' + init.id + ' address=' + JSON.stringify(init.address))
          sendStatus()
        ...
        try {
          ...
          navDiag('nav-init-error', String((error && error.message) || error))
        }
        const navTimer = setInterval(() => {
          navTries += 1
          if (navReady || navTries >= 12) {

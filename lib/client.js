...
`
    ...
    exports.apply = function (ctx) {
      ...
      if (window.parent !== window) {
        ...
        // 到系统主题变化后经外壳 postMessage 转发到这里：偏好为 `system`（或
        // 此前已由本逻辑接管）时写回内置主题（id 与偏好同名：light/dark），
        // 等效恢复跟随；用户在 DSH 里手动选择固定主题时立即让位，改回
        // 「跟随系统」后重新接管。浏览器直开不进入本块（Chromium 原生跟随）。
        let themeTakeover = false
        let themeLastSetByUs = null

        const applySystemTheme = (scheme) => {
          try {
            const theme = ctx.get('theme')
            if (!theme || typeof theme.setTheme !== 'function' || typeof theme.getTheme !== 'function') return
            const snap = theme.getTheme()
            if (!snap) return
            const pref = snap.preference
            if (pref === 'system') {
              themeTakeover = true
            } else if (!(themeTakeover && pref === themeLastSetByUs)) {
              return // 用户手动选了固定主题：让位，直到偏好改回「跟随系统」
            }
            if (snap.active && snap.active.colorScheme === scheme) return
            theme.setTheme(scheme)
            themeLastSetByUs = scheme
          } catch { /* 主题服务异常：忽略，等待下次通知 */ }
        }

        // 用户手动改偏好的让位判定：不是 system、也不是我们刚写的值 → 让位
        const onThemeChangeForHandover = () => {
          try {
            const theme = ctx.get('theme')
            const snap = theme && typeof theme.getTheme === 'function' ? theme.getTheme() : null
            if (!themeTakeover || !snap) return
            const pref = snap.preference
            if (pref === 'system' || pref === themeLastSetByUs) return
            themeTakeover = false
            themeLastSetByUs = null
          } catch { /* 忽略 */ }
        }
        try {
          ctx.on('theme/change', onThemeChangeForHandover)
        } catch { /* 事件不可用时保持接管策略 */ }
        ...
        // 初始接管判定：theme/change 只在变化时发出，启动时主动读一次偏好
        const syncTakeover = () => {
          try {
            const theme = ctx.get('theme')
            const snap = theme && typeof theme.getTheme === 'function' ? theme.getTheme() : null
            if (snap && snap.preference === 'system') themeTakeover = true
          } catch { /* 服务未就绪，走重试 */ }
        }
        syncTakeover()
        let takeoverTries = 0
        const takeoverTimer = setInterval(() => {
          takeoverTries += 1
          if (themeTakeover || takeoverTries >= 12) clearInterval(takeoverTimer)
          else syncTakeover()
        }, 500)
        ctx.effect(() => () => {
          clearInterval(takeoverTimer)
          window.removeEventListener('message', onSystemThemeMessage)
        }, 'dsh-tauri-launcher: system theme follow')
